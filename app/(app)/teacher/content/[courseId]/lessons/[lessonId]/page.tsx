...
import { UploadContentFileForm } from "@/components/content/content-forms";
...
export default async function TeacherLessonPage({ params }: PageProps) {
  ...
  return (
    <div dir={en ? "ltr" : "rtl"} className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>{en ? "Resources" : "المصادر"}</CardTitle></CardHeader><CardContent className="space-y-4"><UploadContentFileForm courseId={course.id} lessonId={lesson.id} />{lesson.video_url && <a href={lesson.video_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-primary hover:underline"><Video className="h-4 w-4" />{en ? "Open lesson video" : "فتح فيديو الدرس"}</a>}{files.length === 0 ? <p className="py-6 text-sm text-muted-foreground">{en ? "No files attached yet." : "لا توجد ملفات مرفقة بعد."}</p> : files.map((file) => <a key={file.id} href={file.download_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-lg border p-3 text-sm hover:bg-accent"><FileText className="h-4 w-4 text-primary" />{file.name}<span className="ms-auto text-xs text-muted-foreground">{Math.ceil(file.size / 1024)} KB</span></a>)}</CardContent></Card>
    </div>
  );
}
