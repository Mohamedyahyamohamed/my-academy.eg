...
import { qrSessionMatchesRequest, verifyQrSession } from "@/lib/qr-session";
...
export async function POST(req: NextRequest) {
  ...
  if (!user || user.role !== "STUDENT" || !user.academy_id) {
    return NextResponse.json({ ok: false, errorCode: "UNAUTHENTICATED", error: "Must be logged in as a student." }, { status: 401 });
  }
  ...
  const requestedLessonId = typeof body.lessonId === "string" ? body.lessonId : "";
  if (!token) {
    return NextResponse.json({ ok: false, errorCode: "QR_INVALID", error: "A valid QR token is required." }, { status: 403 });
  }

  const session = verifyQrSession(token);
  if (!session || !qrSessionMatchesRequest(session, user.academy_id, requestedLessonId)) {
    return NextResponse.json({ ok: false, errorCode: "QR_INVALID", error: "QR token invalid or expired." }, { status: 403 });
  }
  ...
  if (!student) {
    return NextResponse.json({ ok: false, errorCode: "STUDENT_PROFILE_MISSING", error: "No student profile linked." }, { status: 403 });
  }
  ...
  if (!lesson) return NextResponse.json({ ok: false, errorCode: "LESSON_NOT_FOUND", error: "Lesson not found." }, { status: 404 });

  // QR self-check-in is valid only while the lesson is in progress.
  if (!isLessonActive(lesson)) {
    return NextResponse.json({ ok: false, errorCode: "NO_ACTIVE_LESSON", error: "This lesson is not active right now." }, { status: 403 });
  }
  ...
  if (!enrolled) {
    return NextResponse.json({ ok: false, errorCode: "STUDENT_NOT_ENROLLED", error: "You are not enrolled in this class." }, { status: 403 });
  }
  ...
  if (existing) {
    return NextResponse.json({ ok: false, errorCode: "ATTENDANCE_ALREADY_RECORDED", error: "You already checked in for this lesson." }, { status: 409 });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, errorCode: "CHECKIN_FAILED", error: "Could not record attendance." }, { status: 500 });
  }
}
