...
import { createBillingNotifications } from "@/services/billing-notifications";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await createBillingNotifications(client);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    console.error("Billing notification cron failed", error);
