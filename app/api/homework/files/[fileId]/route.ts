...
export async function GET(
  ...
  try {
    const file = await getAuthorizedHomeworkFile(fileId, user);
    if (!file) return NextResponse.json({ error: "File not found" }, { status: 404 });

    const client = nodeSupabaseClient();
    if (!client) return NextResponse.json({ error: "Storage is not configured" }, { status: 503 });

    const signed = await client.storage.from("homework").createSignedUrl(file.storage_path, 60);
    ...
    console.error("[homework-download] denied or unavailable:", error instanceof Error ? error.message : error);
