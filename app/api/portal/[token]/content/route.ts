...
export async function GET(request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  const { data: course } = await client.from("content_courses").select("id,academy_id,group_id,is_published").eq("id", file.course_id).eq("academy_id", student.academy_id).eq("is_published", true).maybeSingle();
