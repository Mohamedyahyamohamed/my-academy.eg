      ...
      <header className="sticky top-0 z-40 border-b border-border/70 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex h-[4.75rem] max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <LanguageToggle />
            <Button asChild variant="ghost" size="sm" className="hidden sm:inline-flex"><Link href="/login">{en ? "Sign in" : "تسجيل الدخول"}</Link></Button>
            <Button asChild size="sm"><Link href="/signup">{en ? "Start free" : "ابدأ مجانًا"}<ArrowLeft className="ms-1 h-4 w-4" /></Link></Button>
          </div>
        </div>
      </header>
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-14 px-4 py-16 sm:px-6 lg:grid-cols-[1.02fr_.98fr] lg:py-24">
            <Reveal>
              <div className="max-w-2xl">
                ...
                <p className="mt-6 max-w-xl text-base leading-8 text-muted-foreground sm:text-lg">{en ? `${APP_CONFIG.name} brings learning, attendance, family communication, and business intelligence into one calm workspace.` : `${APP_CONFIG.name} تجمع التعليم والحضور والتواصل مع الأسرة وذكاء الأعمال داخل مساحة واحدة هادئة وواضحة.`}</p>
                <div className="mt-8 flex flex-wrap gap-3">
                  <Button asChild size="lg" className="shadow-lg shadow-primary/20"><Link href="/signup">{en ? "Create your academy" : "أنشئ أكاديميتك"}<ArrowUpLeft className="ms-1 h-4 w-4" /></Link></Button>
                  <Button asChild size="lg" variant="outline"><Link href="/login">{en ? "Student & parent portal" : "دخول الطلاب وأولياء الأمور"}<ChevronLeft className="ms-1 h-4 w-4" /></Link></Button>
                  {demoEnabled && <DemoLoginButton email="admin@myacademy.edu" password="demo1234" label={en ? "Try the demo" : "جرّب العرض التجريبي"} variant="outline" />}
                </div>
            ...
            <Reveal delay={0.1}>
              <div className="relative mx-auto w-full max-w-xl">
                ...
                <div className="relative rounded-[1.75rem] border border-white/70 bg-card/90 p-4 shadow-2xl shadow-primary/10 backdrop-blur sm:p-5">
