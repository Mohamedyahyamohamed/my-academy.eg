          ...
          <div className="mx-auto grid max-w-7xl items-center gap-14 px-4 py-16 sm:px-6 lg:grid-cols-[1.02fr_.98fr] lg:py-24">
            <Reveal>
              <div className="max-w-2xl">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Button asChild size="lg" className="shadow-lg shadow-primary/20"><Link href={authenticated ? destination : "/signup"}>{authenticated ? ctaLabel : (en ? "Create your academy" : "أنشئ أكاديميتك")}<ArrowLeft className="ms-1 h-4 w-4 rtl:-scale-x-100" /></Link></Button>
                  <Button asChild size="lg" variant="outline" className="border border-slate-200 bg-white text-slate-800 shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"><Link href={authenticated ? destination : "/login"}>{authenticated ? ctaLabel : (en ? "Student & parent portal" : "دخول الطلاب وأولياء الأمور")}<ChevronLeft className="ms-1 h-4 w-4 rtl:-scale-x-100" /></Link></Button>
            ...
            <Reveal delay={0.1}>
              <HeroReveal delay={0.12} className="hero-dashboard-float isolate relative mx-auto w-full max-w-xl">
                <div className="relative z-10 rounded-[1.75rem] border border-slate-100/50 bg-card/90 p-4 shadow-2xl shadow-indigo-500/10 ring-1 ring-gray-900/5 transition-[transform,box-shadow] hover:-translate-y-2 hover:scale-[1.02] backdrop-blur dark:border-slate-800/60 sm:p-5">
