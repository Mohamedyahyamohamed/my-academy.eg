...
export default function StatusPage() {
  ...
  return (
    <main dir={en ? "ltr" : "rtl"} className="min-h-screen bg-background">
      <header className="border-b border-border/70 bg-background/85 backdrop-blur-xl"><div className="mx-auto flex min-h-[4.5rem] h-auto max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><Logo /><div className="flex items-center gap-2"><LanguageToggle /><Button asChild variant="ghost" size="sm" className="hidden sm:inline-flex"><Link href="/">{en ? "Home" : "الرئيسية"}</Link></Button><Button asChild size="sm"><Link href="/signup">{en ? "Start for free" : "ابدأ مجانًا"} <ArrowRight className="h-4 w-4" /></Link></Button></div></div></header>
      <section className="relative isolate overflow-hidden border-b border-border/60 py-16 sm:py-24"><div className="absolute -left-24 -top-24 -z-10 h-80 w-80 rounded-full bg-brand-200/40 blur-3xl" /><div className="absolute -bottom-40 -right-20 -z-10 h-96 w-96 rounded-full bg-sky-100/70 blur-3xl" /><div className="mx-auto max-w-4xl px-4 text-center sm:px-6"><p className="text-sm font-bold text-brand-600">{en ? "Operational transparency" : "شفافية تشغيلية"}</p><h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{en ? "MY Academy service status" : "حالة خدمة MY Academy"}</h1><p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-muted-foreground sm:text-lg">{en ? "Check application and database availability directly. We show public indicators only and never expose private keys or data." : "تحقق من توفر التطبيق وقاعدة البيانات مباشرة. نعرض مؤشرات عامة فقط ولا نكشف أي مفاتيح أو بيانات خاصة."}</p><div className={`mx-auto mt-10 flex max-w-2xl flex-wrap items-start gap-4 rounded-2xl border p-5 text-right shadow-sm ${statusTone}`}><StatusIcon className="mt-0.5 h-6 w-6 shrink-0" /><div className="min-w-0 flex-1"><h2 className="font-black">{statusCopy.title}</h2><p className="mt-1 text-sm leading-6 opacity-80">{statusCopy.text}</p>{health?.latency_ms !== undefined && <p className="mt-2 text-xs opacity-70">{en ? "Check latency:" : "زمن الفحص:"} {health.latency_ms} {en ? "ms" : "مللي ثانية"}</p>}</div><Button type="button" variant="outline" size="sm" onClick={() => void checkStatus()} disabled={state === "checking"} aria-label={en ? "Refresh service status" : "تحديث حالة الخدمة"} title={en ? "Refresh service status" : "تحديث حالة الخدمة"} className="ms-auto shrink-0 bg-white/70"><RefreshCw className={`h-4 w-4 ${state === "checking" ? "animate-spin" : ""}`} /></Button></div></div></section>
      ...
      <footer className="border-t border-border py-8"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 text-sm text-muted-foreground sm:px-6"><Logo /><p>© {new Date().getFullYear()} MY Academy. {en ? "All rights reserved." : "جميع الحقوق محفوظة."}</p><div className="flex flex-wrap gap-4"><Link href="/privacy" className="transition hover:text-foreground">{en ? "Privacy" : "الخصوصية"}</Link><Link href="/pricing" className="transition hover:text-foreground">{en ? "Pricing" : "الأسعار"}</Link><Link href="/" className="transition hover:text-foreground">{en ? "Home" : "الرئيسية"}</Link></div></div></footer>
    </main>
  );
}
