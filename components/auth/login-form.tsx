...
export function LoginForm() {
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    try {
      ...
      toast.success(en
        ? <span dir="ltr">Welcome, {firstName}!</span>
        : <span dir="rtl">أهلاً بك، <span dir="ltr">{firstName}</span>!</span>);
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
    ...
    <div className="space-y-1.5"><div className="flex items-center justify-between"><Label className="text-sm font-medium" htmlFor="password">{en ? "Password" : "كلمة المرور"}</Label><a href="/forgot-password" className="text-xs font-medium text-primary transition-colors hover:text-indigo-700 hover:underline dark:hover:text-indigo-300">{en ? "Forgot password?" : "نسيت كلمة المرور؟"}</a></div><div className="relative"><Lock className={`pointer-events-none absolute top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 dark:text-slate-500 ${en ? "left-3" : "right-3"}`} /><Input id="password" type="password" autoComplete="current-password" placeholder="••••••••" className={`rounded-lg border border-gray-300 bg-white text-slate-900 placeholder:text-gray-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-indigo-400 dark:focus:ring-indigo-400 ${en ? "pl-9" : "pr-9"}`} {...register("password")} /></div>{errors.password && <p className="text-xs leading-5 text-red-500 dark:text-red-400">{errors.password.message}</p>}</div>
