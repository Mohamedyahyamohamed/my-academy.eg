...
export function LoginForm() {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
    <div className="space-y-1.5"><Label className="text-sm font-medium" htmlFor="email">{en ? "Email address" : "البريد الإلكتروني"}</Label><div className="relative"><Mail className={`pointer-events-none absolute top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground ${en ? "left-3" : "right-3"}`} /><Input id="email" type="email" autoComplete="email" placeholder="you@academy.edu" className={en ? "pl-9" : "pr-9"} {...register("email")} /></div>{errors.email && <p className="text-xs leading-5 text-destructive">{errors.email.message}</p>}</div>
    <div className="space-y-1.5"><div className="flex items-center justify-between"><Label className="text-sm font-medium" htmlFor="password">{en ? "Password" : "كلمة المرور"}</Label><a href="/forgot-password" className="text-xs font-medium text-primary hover:underline">{en ? "Forgot password?" : "نسيت كلمة المرور؟"}</a></div><div className="relative"><Lock className={`pointer-events-none absolute top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground ${en ? "left-3" : "right-3"}`} /><Input id="password" type="password" autoComplete="current-password" placeholder="••••••••" className={en ? "pl-9" : "pr-9"} {...register("password")} /></div>{errors.password && <p className="text-xs leading-5 text-destructive">{errors.password.message}</p>}</div>
