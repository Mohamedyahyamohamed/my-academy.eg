...
export function SignupForm() {
  ...
  const [form, setForm] = React.useState({ academyName: "", fullName: "", email: "", password: "" });
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("تم إنشاء أكاديميتك بنجاح.");
      router.refresh();
    } catch {
    ...
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
        <div className="flex items-start gap-3">
          ...
          <p>أنشئ أكاديمية جديدة لتصبح مديرها. لإضافة المدرّسين والطلاب وأولياء الأمور، ستُرسل لهم دعوات شخصية وآمنة من الإعدادات بعد التسجيل.</p>
        </div>
      </div>
      ...
      <Button type="submit" className="w-full" disabled={loading}>
        {loading ? <><Loader2 className="h-4 w-4 animate-spin" />جارٍ إنشاء الأكاديمية…</> : <>إنشاء أكاديميتي <ArrowRight className="h-4 w-4" /></>}
      </Button>
