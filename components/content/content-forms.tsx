...
export function CreateCourseForm({ groups }: { groups: Array<{ id: string; name: string }> }) {
  ...
  const en = lang === "en";
  return (
    <form action={async (formData) => { await createContentCourseAction(formData); }} className="space-y-3 rounded-xl border bg-card p-4">
      ...
      <Button type="submit">{en ? "Create course" : "إنشاء الدورة"}</Button>
    </form>
  );
}
...
export function CreateLessonForm({ courseId }: { courseId: string }) {
  ...
  const en = lang === "en";
  return (
    <form action={async (formData) => { await createContentLessonAction(formData); }} className="space-y-3 rounded-xl border bg-card p-4">
      ...
      <Button type="submit">{en ? "Add lesson" : "إضافة الدرس"}</Button>
    </form>
  );
}
