...
import { createContentCourseAction, createContentLessonAction, markLessonCompleteAction, createContentUploadIntent, finalizeContentUpload, addContentLink } from "@/app/actions/content";
import { createBrowserSupabaseClient } from "@/lib/supabase/client";
import { CONTENT_UPLOAD_ACCEPT, CONTENT_UPLOAD_EXTENSIONS, MAX_CONTENT_UPLOAD_BYTES, MAX_CONTENT_UPLOAD_MB } from "@/lib/upload-policy";

const CONTENT_CLIENT_TIMEOUT_MS = 30_000;

async function withClientTimeout<T>(operation: PromiseLike<T>, label: string): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([
      Promise.resolve(operation),
      new Promise<T>((_, reject) => {
        timer = setTimeout(() => reject(new Error(`${label} timed out. Please try again.`)), CONTENT_CLIENT_TIMEOUT_MS);
      }),
    ]);
  } finally {
...
export function UploadContentFileForm({ courseId, lessonId, lessons = [] }: { courseId: string; lessonId?: string; lessons?: LessonOption[] }) {
  ...
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const MAX_FILE_SIZE = MAX_CONTENT_UPLOAD_BYTES;
  const ALLOWED_EXTENSIONS = new Set<string>(CONTENT_UPLOAD_EXTENSIONS);
  return (
    <form onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const input = form.elements.namedItem("file"); const file = input instanceof HTMLInputElement ? input.files?.[0] : undefined; if (!file) { toast.error(en ? "Choose a file first." : "اختر ملفًا أولًا."); return; } const extension = file.name.split(".").pop()?.toLowerCase() ?? ""; if (!ALLOWED_EXTENSIONS.has(extension)) { toast.error(en ? "This file type is not allowed." : "نوع الملف غير مسموح."); return; } if (file.size <= 0 || file.size > MAX_FILE_SIZE) { toast.error(en ? `File size must be ${MAX_CONTENT_UPLOAD_MB} MB or less.` : `يجب ألا يتجاوز حجم الملف ${MAX_CONTENT_UPLOAD_MB} ميجابايت.`); return; } startTransition(async () => { try { const intentData = new FormData(); intentData.set("courseId", courseId); intentData.set("lessonId", lessonId ?? ""); intentData.set("fileName", file.name); intentData.set("fileSize", String(file.size)); intentData.set("contentType", file.type);       const intent = await withClientTimeout(createContentUploadIntent(intentData), "Creating the upload session"); if (!intent.ok) throw new Error(intent.error); const uploadPath = intent.path; const uploadToken = intent.token; const uploadType = intent.contentType; if (!uploadPath || !uploadToken || !uploadType) throw new Error("Upload intent is incomplete."); const supabase = createBrowserSupabaseClient();       const uploaded = await withClientTimeout(supabase.storage.from("content").uploadToSignedUrl(uploadPath, uploadToken, file, { contentType: uploadType }), "Uploading the file");
      if (uploaded.error) throw new Error(uploaded.error.message || "Upload failed."); const finalizeData = new FormData(); finalizeData.set("courseId", courseId); finalizeData.set("lessonId", lessonId ?? ""); finalizeData.set("path", uploadPath); finalizeData.set("fileName", file.name); finalizeData.set("fileSize", String(file.size)); finalizeData.set("contentType", uploadType);       const result = await withClientTimeout(finalizeContentUpload(finalizeData), "Finalizing the upload"); if (!result.ok) throw new Error(result.error); toast.success(en ? "File uploaded." : "تم رفع الملف."); form.reset(); router.refresh(); } catch (error) { toast.error(error instanceof Error ? error.message : (en ? "Upload failed." : "فشل رفع الملف.")); } }); }} className="flex flex-wrap items-end gap-3 rounded-xl border bg-card p-4">
      <input type="hidden" name="courseId" value={courseId} />
      ...
      <p className="w-full text-xs text-muted-foreground">{en ? `PDF, DOCX, images, or MP4 up to ${MAX_CONTENT_UPLOAD_MB} MB.` : `PDF أو DOCX أو صور أو MP4 حتى ${MAX_CONTENT_UPLOAD_MB} ميجابايت.`}</p><Button type="submit" variant="soft" disabled={pending}>{pending ? (en ? "Uploading…" : "جارٍ الرفع…") : (en ? "Upload file" : "رفع الملف")}</Button>
    </form>
  );
}
